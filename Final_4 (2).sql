...
CREATE TABLE contacts(
	`contactID` INT(4) NOT NULL, #contactID has to have a value
...
SELECT 
    ...
    ctBill.address AS 'Bill To Address',  -- Selects the billing address from the contacts table
    ctBill.phoneNumber AS 'Bill To Phone',  -- Selects the billing phone number from the contacts table
    CONCAT(ctShip.contactFirstName, ' ', ctShip.contactLastName) AS 'Ship To Recipient',  -- Combines first and last name from shipping contact
    ctShip.address AS 'Ship To Address',  -- Selects the shipping address from the contacts table
    ctShip.phoneNumber AS 'Ship To Phone',  -- Selects the shipping phone number from the contacts table
...
LEFT JOIN 
    contacts ctBill ON o.customerID = ctBill.customerID AND ctBill.isbilling = 1  -- Join contacts for billing address where isbilling = 1
LEFT JOIN 
    contacts ctShip ON o.contactID = ctShip.contactID  -- Join contacts for shipping information
LEFT JOIN 
...
ORDER BY 
    od.partID;  -- Order the results by partID to group the items in each order
